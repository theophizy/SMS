...
switch($action){
// add a student for the selected subject if his or her name is not found
 ...
 $totalmarkscode =  $student_regno.$students_class.$students_classarm.$session.$student_term;
	 ...
	 if(isset($getScoresCode) && $getScoresCode > 0) {
		   header("location:../Admin/input_scores.php?err=3&student_term=$student_term&students_class=$students_class&session=$session&students_classarm=$students_classarm&students_subjects=$students_subjects");
...
$database->InsertOrUpdate("INSERT INTO `sms_average_scores`(`avg_code`,`student_no`,`avg_class`,`avg_class_arm`,`avg_term`,`avg_session`) VALUES('$totalmarkscode','$student_regno','$students_class','$students_classarm','$student_term','$session')");
	 ...
	 header("location:../Admin/input_scores.php?emsg=1&student_term=$student_term&students_class=$students_class&session=$session&students_classarm=$students_classarm&students_subjects=$students_subjects");
	exit;
  }else{
...
case "enterStudentscores":
	...
	if (isset($_POST['send'])){
			...
			//bind values to variables
		$score_code = $_POST['scorecode']; 
	
		$students_class = $_POST['students_class']; 
		
	   $students_classarm = $_POST['students_classarm']; 
	
		$student_term = $_POST['student_term']; 
	
		$students_subjects = $_POST['students_subjects'];
		$session = $_POST['session']; 
		$student_reg = $_POST['regno']; 
		
	 $avg_code =  $student_reg.$students_class.$students_classarm.$session.$student_term;
	$score_by = $_SESSION['USERNAME'];
	
		$score_date = date('Y/m/d');
		 $score_CA1 = $_POST["CA1"];
	  $score_CA2 = $_POST["CA2"];
	   $score_CA3 = $_POST["CA3"];
	    $score_exam = $_POST["exams"];
		
	
 $totalscores = floor($score_CA1 + $score_CA2 + $score_CA3 + $score_exam);
 $studentgrade = $getgrade->getStudentGrade($totalscores);
	
	

   $insetid = $create_scores->updatestudentscoresforeachsubject($score_code,$score_CA1,$score_CA2,$score_CA3,$score_exam,$totalscores,$studentgrade,$score_by,$score_date);
	
	 }
  if($insetid == true){
	$totalmarksobtaindebystudent = $create_scores->getStudentTotalMarksObtainedinExamswithParameters($student_reg,$students_class,$students_classarm,$student_term,$session);
	$create_scores->updatestudenttotalscoresobtained($avg_code,$totalmarksobtaindebystudent);
	 header("location:../Admin/input_scores.php?emsg=1&student_term=$student_term&students_class=$students_class&session=$session&students_classarm=$students_classarm&students_subjects=$students_subjects");
	exit;
  }else{
