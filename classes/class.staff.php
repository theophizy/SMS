...
'".$this->Escape($this->staff_status)."')";	
	...
	function AvoidStaffDuplicate(){
		$this->query = "SELECT `staff_id` FROM ".$this->table." WHERE `staff_id` = '".$this->staff_id."'";
		...
		$this->query = "SELECT * FROM ".$this->table."  WHERE `staff_status`='ACTIVE'";
...
// view form teacher by class

function viewFormasterbyClass($class,$classarm){
		$this->query = "SELECT formaster_staffid FROM sms_formaster WHERE  `formaster_class_code` = '$class' AND `formaster_class_arm` = '$classarm'";
		foreach($this->Resultset($this->query) as $key => $value){
		
	$form_teacher = $value['formaster_staffid'];
