...
$delete = $this->InsertOrUpdate($this->q);	
	...
	function Save(){
	$this->query = "SELECT `student_regno`,image 
	FROM
	 ".$this->table."
	  WHERE 
	  `student_regno`=
...
'".$this->Escape($this->student_status)."')";	
	...
	public function generateMatrixNumber($year){
			 
			$que = "SELECT COUNT(`student_regno`) FROM ".$this->table."";
			//invokin a method from Database class that returns number of rows
			$stmt = mysqli_query($this->connection,$que);
			$rows = mysqli_fetch_array($stmt);
		
		$numberOfStudents = $rows[0] + 1;
			
			 if($numberOfStudents < 1){
				 $matrixNo  = $year."0000". $numberOfStudents;
			 }
				
		     elseif($numberOfStudents < 10){
				 $matrixNo  = $year."0000". $numberOfStudents;
			 }elseif($numberOfStudents < 100){
				 $matrixNo  = $year."000".$numberOfStudents;
			 }elseif($numberOfStudents < 1000){
				 $matrixNo  = "00".$numberOfStudents;
			 }elseif($numberOfStudents < 10000){
				 $matrixNo  = $year."0".$numberOfStudents;
			 }else {
				  $matrixNo  = $year.$numberOfStudents;
			 }
			return $matrixNo ;
