...
class Scores extends Term{
	...
	public $formaster_table = "sms_formaster";
	public $avg_class;
...
function getStudentscoresheet(){
 ...
 return $this->Resultset($this->query);
	...
	function view_scoresbyTermSessionClass(){
			$this->query = "SELECT score_code,score_student_regno,score_subject_code,
score_class,
score_class_arm,
score_CA1,
score_CA2,
 ...
 return $this->Resultset($this->query);
	...
	//view class result function
		function view_class_result(){
			$this->query = "SELECT DISTINCT(class_sub_subject_code) FROM 
			sms_class_subjects
			  WHERE
			   `class_sub_class_code` = '".$this->Escape($this->score_class)."'";
...
function number_subjects_student_enrolled(){
	...
	$total_subjects_student =0;
	
$this->query = "SELECT `score_subject_code`,`score_student_regno`,`score_class`,`score_class_arm`,`score_term`,`score_session`  FROM 
			 " .$this->table."
			  WHERE
			   `score_class` = '".$this->Escape($this->score_class)."'
			    AND 
 ...
 foreach($this->Resultset($this->query) as $key => $val){	
   ...
   $total_subjects = $this->number_subjects_student_enrolled_per_term();
...
function updatestudentscoresforeachsubject($score_code,$score_CA1,$score_CA2,$score_CA3,$score_exam,$score_total,$score_grade,$score_by,$score_date){
$this->query =  "UPDATE  ".$this->table." SET score_CA1='$score_CA1',score_CA2='$score_CA2',score_CA3='$score_CA3',score_exam='$score_exam',score_total='$score_total',score_grade='$score_grade',score_by='$score_by',score_date='$score_date' WHERE `score_code` ='$score_code'";
$insert = $this->InsertOrUpdate($this->query);
return $insert;
}	

function getFormTeacher($classcode,$classarm){
	$this->query = "SELECT sms_formaster.formaster_staffid,sms_staff.staff_first_name,sms_staff.staff_last_name FROM sms_formaster JOIN sms_staff ON sms_formaster.formaster_staffid = sms_staff.staff_id WHERE sms_formaster.formaster_class_code = '$classcode' AND sms_formaster.formaster_class_arm ='$classarm'";
	 foreach($this->Resultset($this->query) as $key => $val){
	 $formaster_name = $val['staff_first_name']."   ".$val['staff_last_name'];	 
 }
 return $formaster_name;
}
