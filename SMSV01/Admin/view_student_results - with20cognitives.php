...
$studentAction->student_regno = $_GET['admission_no'];
...
$student_term = $_GET['student_term'];
 ...
 // $avg_code = $_GET['admission_no'].$_GET['students_class'].$students_classarm.$session.$student_term;
  $avg_code =  $_GET['admission_no'].$students_class.$students_classarm.$session.$student_term;
...
$classarm->viewClassArmByID();	   
	...
	$createscore->score_student_regno = $_GET['admission_no'];
...
$total_subjects_by_student = $createscore->number_subjects_student_enrolled();
	...
	$student_class_postion = $createscore->position_in_class($_GET['admission_no']);
	...
	$school_setup->cog_student_reg = $_GET['admission_no'];
...
<center>
	...
	<td>
            ...
            <tr><td><table style="width:100%;" border="1">
				<tr><td colspan="2">NAME:<?php echo $studentAction->last_name." ".$studentAction->first_name ?></td>&nbsp;&nbsp;<td colspan="2">ADMISSION NO:<?php echo $_GET['admission_no'] ?></td><td colspan="2">SEX:<?php echo $studentAction->sex ?></td><td colspan="3">TOTAL MARKS OBTAINED:<?php echo $createscore->avg_total_marks ?></td></tr>
            ...
            <td colspan="2">STUDENT AVERAGE:<?php echo number_format(($createscore->avg_total_marks/$total_subjects_by_student ),1)."%";?></td></tr>
				<tr><td colspan="2">CLASS:<?php echo $studentclass->class_name. $classarm->class_arm_name?></td> <td colspan="2">NUMBER IN CLASS:<?php echo $total_students; ?></td><td colspan="2">NEXT TERM BEGIN:<?php foreach($school_setup->viewNextTermBegin() as $key => $val){ echo $val['next_date'];}  ?></td><td>ATTENDANCE:<?php $attendance = (($createscore->attendance /$createscore->days_school_opened)*100); echo number_format($attendance,1)."%" ?></td></tr>
	...
	$sn=0;
		...
		$position = $createscore->position_in_subject($_GET['admission_no']);
		//get position's suffix like rd for 3rd or nd for 2nd
		$suffix = $createscore->print_suffix($position);
		
		 ?>
				<tr bgcolor="<?php echo $bg_color  ?>"><td><?php echo $sn; ?></td><td><?php echo ucwords($subject->subject_name) ?></td><td align="center"><?php echo $values['score_CA1']  ?></td><td align="center"><?php echo $values['score_CA2']  ?></td><td align="center"><?php echo $values['score_CA3']  ?></td><td align="center"><?php echo $values['score_exam']  ?></td><td align="center"><?php echo $values['score_total']  ?></td><td align="center"><?php echo $maximun_score; ?></td><td align="center"><?php echo $minimum_score; ?></td><td align="center"><?php echo $position  ?><sup><?php echo $suffix  ?></sup></td><td align="center"><?php echo $getgrade->grade_name; ?></td><td><?php echo $getgrade->grade_remark; ?></td></tr>
    ...
    <tr>
		<td>
            ...
            <tr>
               <td>
                ...
                <tr bgcolor="#ddd"><td>COGNITIVE SKILL</td><td>1</td><td>2</td><td>3</td><td>4</td><td>5</td></tr>
                 <?php 
				if(!empty($school_setup->viewAllCognitiveSkillswithOffset(5,0))){
				foreach($school_setup->viewAllCognitiveSkillswithOffset(5,0) as $key => $val){ 
			...
			?>
               ...
               <?php	
						}}else{ echo "YOUR COGNITIVE SKILLS ARE NOT YET ASSESSED BY YOUR FORMASTER";}
								?>
               </table>
               </td>
                <td>
                ...
                <tr bgcolor="#ddd"><td>COGNITIVE SKILL</td><td>1</td><td>2</td><td>3</td><td>4</td><td>5</td></tr>
                 <?php 
				if(!empty($school_setup->viewAllCognitiveSkillswithOffset(5,5))){
				foreach($school_setup->viewAllCognitiveSkillswithOffset(5,5) as $key => $val){ $cognitive_id = $val['cognitive_id'];
				$school_setup->cog_id = $cognitive_id;
				$school_setup->viewCog_student_skills();
			$item_selected = $val['cog_student_item'];
			?>
               ...
               <?php	
                ...
                <tr bgcolor="#ddd"><td>COGNITIVE SKILL</td><td>1</td><td>2</td><td>3</td><td>4</td><td>5</td></tr>
                 <?php 
				if(!empty($school_setup->viewAllCognitiveSkillswithOffset(5,10))){
				foreach($school_setup->viewAllCognitiveSkillswithOffset(5,10) as $key => $val){ $cognitive_id = $val['cognitive_id'];
				$school_setup->cog_id = $cognitive_id;
				$school_setup->viewCog_student_skills();
			$item_selected = $val['cog_student_item'];
			?>
               ...
               <?php	
                ...
                <tr bgcolor="#ddd"><td>COGNITIVE SKILL</td><td>1</td><td>2</td><td>3</td><td>4</td><td>5</td></tr>
                 <?php 
				if(!empty($school_setup->viewAllCognitiveSkillswithOffset(5,15))){
				foreach($school_setup->viewAllCognitiveSkillswithOffset(5,15) as $key => $val){ $cognitive_id = $val['cognitive_id'];
				$school_setup->cog_id = $cognitive_id;
				$school_setup->viewCog_student_skills();
				$item_selected = $val['cog_student_item'];
			?>
               ...
               <tr bgcolor="#CCC"><td><font size="+1"><i>Form Master's Remark:</i></font></td><td colspan="4"><?php echo ucwords($student_remark) ?></td></tr> 
			</table>   
		</td>
		</tr></table>
