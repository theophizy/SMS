 ...
 <div class="col-lg-4">
   ...
   if(isset($_POST['search'])){
	  ...
	  //bind selected subject id to its variable and pass it into view subject a by its id to get the name of the subject 
		...
		if(($_POST['term']==0)){
			 header("location:resultsheet_student.php?err=16");
		}else
		if(empty($createscore->view_scoresbystudent())){
				 header("location:resultsheet_student.php?err=15");
			
		}
		else{
			
	header("location:resultsheet1.php?student_term=$student_term&students_class=$students_class&session=$session&students_classarm=$students_classarm");
		}
		}
   ?>
