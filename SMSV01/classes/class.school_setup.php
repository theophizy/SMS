...
function sch_setup($school_id = '', $school_name = '', $school_address = '', 	 $school_logo = '', $school_phone = '', $school_email = '',$school_status = 'ACTIVE', $cog_id = '', $cog_name = '',    	 $cog_status = 'ACTIVE',$cog_student_id = '',$cog_student_reg = '',$cognitive_id = '',$cog_student_item = '',$cog_student_class = '',$cog_student_session = '',$cog_student_term = '',$cog_student_status ='ACTIVE', $next_id = '' , $next_date = '', $duration_id = '', $duration_term = '', $duration_session = '', $start_date = '', $end_date = '', $remark_code = '', $remark_min_mark = '', $remark_max_mark = '', $remark = '', $remark_term = '', $remark_session = '' ){
		...
		$this->remark = $remark;
		$this->remark_term = $remark_term;
		$this->remark_session = $remark_session;
		}
...
foreach($this->Resultset($this->query) as $key => $value){
 ...
 function AvoidScoreDurationDuplicate(){
	...
	if($number_rows > 0){
		...
		$this->query = "UPDATE ".$this->formmaster_remark_table." SET `remark_min_mark`='".$this->Escape($this->remark_min_mark)."',`remark_max_mark`='".$this->Escape($this->remark_max_mark)."',`remark_term`='".$this->Escape($this->remark_term)."',`remark_session`='".$this->Escape($this->remark_session)."',`remark`='".$this->Escape($this->remark)."' WHERE `remark_code`='".$this->remark_code."'";	
		
		
	}else{
	$this->query = "INSERT INTO ".$this->formmaster_remark_table."(`remark_min_mark`,`remark_max_mark`,`remark`,`remark_term`,`remark_session`) VALUES('".$this->Escape($this->remark_min_mark)."','".$this->Escape($this->remark_max_mark)."','".$this->Escape($this->remark)."','".$this->Escape($this->remark_term)."','".$this->Escape($this->remark_session)."')";	
	}
	$insertid = $this->InsertOrUpdate($this->query);
	return $insertid;
	}
	...
	function view_form_MastersRemarkByTermandSession(){
			$this->query = "SELECT * FROM 
			 " .$this->formmaster_remark_table."
			  WHERE
			    `remark_term`='".$this->Escape($this->remark_term)."' AND `remark_session`='".$this->Escape($this->remark_session)."'";
...
function viewAllFormmasterRemark(){
 ...
 return $this->Resultset($this->query);
	...
	if($number_rows > 0){
		...
		$this->query = "UPDATE ".$this->principal_remark_table." SET `remark_min_mark`='".$this->Escape($this->remark_min_mark)."',`remark_max_mark`='".$this->Escape($this->remark_max_mark)."',`remark_term`='".$this->Escape($this->remark_term)."',`remark_session`='".$this->Escape($this->remark_session)."',`remark`='".$this->Escape($this->remark)."' WHERE `remark_code`='".$this->remark_code."'";	
		
		
	}else{
	$this->query = "INSERT INTO ".$this->principal_remark_table."(`remark_min_mark`,`remark_max_mark`,`remark`,`remark_term`,`remark_session`) VALUES('".$this->Escape($this->remark_min_mark)."','".$this->Escape($this->remark_max_mark)."','".$this->Escape($this->remark)."','".$this->Escape($this->remark_term)."','".$this->Escape($this->remark_session)."')";	
	}
	$insertid = $this->InsertOrUpdate($this->query);
	return $insertid;
	}
