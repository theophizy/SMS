...
$studentNo = $_GET['admission_no'];
$lengthNumber  = strlen((string)$studentNo);

if($lengthNumber <= 5){
$numofZeros = 5;
}else{
	$numofZeros = $lengthNumber;
	}
	$student_reg_number = str_pad((string)$_GET['admission_no'] , $numofZeros , "0" , STR_PAD_LEFT);
$studentAction->student_regno = $student_reg_number;
//echo"====".$_GET['admission_no']."==".$lengthNumber."==padded==".$studentAction->student_regno;
...
$student_term = $_GET['student_term'];
 ...
 // $avg_code = $_GET['admission_no'].$_GET['students_class'].$students_classarm.$session.$student_term;
  $avg_code =  $student_reg_number.$students_class.$students_classarm.$session.$student_term;
  $createscore->avg_code = $avg_code;
  $createscore->viewStudentTotalMarksObtainedByRegno();
  //get principal
  $principal_name = $school_setup->getPrincipal();
 // get formaster name
$formaster_name =  $createscore->getFormTeacher($students_class,$students_classarm);
...
$classarm->viewClassArmByID();	   
	...
	$createscore->score_student_regno = $student_reg_number;
...
$getminimumandmaximumsubjectslimit = $subject->viewAllSubjectLimitbyClassTermandSessiontobeOffered();
	...
	$student_class_postion = $createscore->position_in_class($student_reg_number);
	...
	$school_setup->cog_student_reg = $student_reg_number;
...
<center>
	...
	<td>
            ...
            <tr><td><table style="width:100%;" border="1">
				<tr><td colspan="2">Name:<?php echo $studentAction->last_name." ".$studentAction->first_name ?></td>&nbsp;&nbsp;<td colspan="2">Admission No:<?php echo $student_reg_number ?></td><td colspan="2">SEX:<?php echo $studentAction->sex ?></td><td colspan="3">Total Marks Obtained:<?php echo $createscore->avg_total_marks ?></td></tr>
	...
	$sn=0;
		...
		$position = $createscore->position_in_subject($student_reg_number);
		...
		<td>
			...
			?>
             ...
             <tr><td><font size="+1"><i>Maximum Number of Subjects :</i></font><b><?php echo $getminimumandmaximumsubjectslimit->max_limit ?></b></td><td><font size="+1"><i>Minimum Number of Subjects:</i></font><b><?php echo $getminimumandmaximumsubjectslimit->min_limit ?></b></td><td><font size="+1"><i>Number of Subject Offered:</i></font><b><?php echo $numberofsubjectsstudentoffered; ?></b></td></tr> 
               <tr><td><font size="+1"><i>Form Master's Remark:</i></font></td><td colspan="2"><?php echo ucwords(strtolower($student_remark)) ?></td><td><font size="+1"><i>Formaster:</i></font><b><?php echo $formaster_name; ?></b></td></tr> 
                <tr><td><font size="+1"><i>Principal's Remark:</i></font></td><td colspan="2"><?php echo ucwords(strtolower($principal_remark)); ?></td><td><font size="+1"><i>Principal:</i></font><b><?php echo $principal_name; ?></b></td></tr> 
			</table>   
		</td>
		</tr></table>
